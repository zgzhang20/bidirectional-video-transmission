...
class AF_Module_channle_1(nn.Module):  # 合成图降维
    def __init__(self, channel):
        super(AF_Module_channle_1, self).__init__()
...
class recons_mid_forw(nn.Module):  # 原图降维
    def __init__(self, ):
        ...
        # self.igdn2 = GDN(128, device, inverse=True)
        self.igdn3 = GDN(256, inverse=True)
        # self.igdn4 = GDN(128, device, inverse=True)
        self.igdn5 = GDN(64, inverse=True)
        # self.igdn6 = GDN(32, device, inverse=True)
        # self.igdn7 = GDN(3, device, inverse=True)
...
def train_pyconv(model, kye_model, opt, bimage, channel, n_var):
    ...
    top_image_F = model.FeatureEncoder(top_image)
    bottom_image_F = model.FeatureEncoder(bottom_image)
    key_feature = model.FeatureEncoder(key)
    input_feature = model.FeatureEncoder(predict_feature)
    ...
    loss = 100 - psnr(mid * 255, pre_frame * 255)
...
def test_epoch(model, key_pret_model, bimage, channel, n_var):
    model.eval()
    key, ssim_value, psn, mse, key_send = test_pyconv(key_pret_model, bimage, channel, n_var)
    ...
    mse = torch.mean(torch.square(pre_frame * 255 - mid * 255))
    ms_ssim_module = MS_SSIM(data_range=255, size_average=True, channel=3)
    loss = ms_ssim_module(mid * 255, pre_frame * 255)
    ssim_value = loss.data.item()
    psn = psnr(pre_frame * 255, mid * 255)

    return mid, ssim_value, psn, mse
